...
exports.getDashboardStats = exports.updateLeadStatus = exports.getSaleEvents = exports.saleEvents = exports.getLeads = exports.createLead = void 0;
...
const createLead = async (req, res) => {
    try {
        ...
        if (!corretorDestinoId && repasse_id) {
            const { rows: repasseObj } = await db.query('SELECT corretor_id FROM repasses WHERE id = $1', [parseInt(repasse_id)]);
        ...
        if (!corretorDestinoId) {
            const { rows: ultimoLead } = await db.query('SELECT corretor_id FROM leads WHERE corretor_id IS NOT NULL ORDER BY id DESC LIMIT 1');
...
const getSaleEvents = async (req, res) => {
    res.json(exports.saleEvents);
};
exports.getSaleEvents = getSaleEvents;
...
const updateLeadStatus = async (req, res) => {
    ...
    `;
        ...
        if (status === 'Vendido' && lead.repasse_id) {
            await db.query("UPDATE repasses SET status = 'Vendido' WHERE id = $1", [lead.repasse_id]);
            // Disparar evento de venda para toda a equipe
            try {
                const details = await db.query(`SELECT c.nome as corretor_nome, r.titulo as repasse_titulo 
           FROM corretores c
           CROSS JOIN repasses r
           WHERE c.id = $1 AND r.id = $2`, [lead.corretor_id, lead.repasse_id]);
                if (details.rows.length > 0) {
                    const brokerName = details.rows[0].corretor_nome || 'Um corretor';
                    const propertyTitle = details.rows[0].repasse_titulo || 'um imóvel';
                    exports.saleEvents.push({
                        id: `sale-${id}-${Date.now()}`,
                        brokerName,
                        propertyTitle,
                        timestamp: Date.now()
                    ...
                    if (exports.saleEvents.length > 20) {
                        exports.saleEvents.shift();
                    }
                }
            }
            catch (err) {
            ...
            await db.query("UPDATE repasses SET status = 'Disponível' WHERE id = $1 AND status = 'Vendido'", [lead.repasse_id]);
        }
        res.json(lead);
    }
    catch (err) {
...
const getDashboardStats = async (req, res) => {
    ...
    `;
        let vgvParams = [pctCorretorPadrao];
        if (isCorretor) {
    ...
    `;
      ...
      WHERE r.status = 'Vendido'
    `;
        let paramVendas = [pctCorretorPadrao];
        if (isCorretor) {
        ...
        if (isCorretor) {
            ...
            const captacoesRes = await db.query("SELECT COUNT(*), COALESCE(SUM(valor_chave * (COALESCE(comissao_pct, $1) / 100.0)), 0) as comissao_pendente FROM repasses WHERE corretor_id = $2 AND status = 'Disponível'", [pctCorretorPadrao, corretorId]);
            ...
            const aprovacoesRes = await db.query("SELECT COUNT(*) FROM leads WHERE corretor_id = $1 AND status = 'Aprovado'", [corretorId]);
            additionalStats.aprovacoes = parseInt(aprovacoesRes.rows[0].count);
            // Vendas do corretor
            const vendasRes = await db.query("SELECT COUNT(*) FROM leads WHERE corretor_id = $1 AND status = 'Vendido'", [corretorId]);
            additionalStats.vendas = parseInt(vendasRes.rows[0].count);
            // Pendências do corretor
            const pendenciasRes = await db.query("SELECT COUNT(*) FROM leads WHERE corretor_id = $1 AND status IN ('Novo', 'Não respondeu', 'Em negociação')", [corretorId]);
            additionalStats.pendencias = parseInt(pendenciasRes.rows[0].count);
        }
        else {
            // Captações ativas globais
            const captacoesRes = await db.query("SELECT COUNT(*), COALESCE(SUM(valor_chave * (COALESCE(comissao_pct, $1) / 100.0)), 0) as comissao_pendente FROM repasses WHERE status = 'Disponível'", [pctCorretorPadrao]);
            additionalStats.captacoes = parseInt(captacoesRes.rows[0].count);
            additionalStats.comissaoPendente = parseFloat(captacoesRes.rows[0].comissao_pendente);
            // Aprovações globais
            const aprovacoesRes = await db.query("SELECT COUNT(*) FROM leads WHERE status = 'Aprovado'");
            additionalStats.aprovacoes = parseInt(aprovacoesRes.rows[0].count);
            // Vendas globais
            const vendasRes = await db.query("SELECT COUNT(*) FROM leads WHERE status = 'Vendido'");
            additionalStats.vendas = parseInt(vendasRes.rows[0].count);
            // Pendências globais
            const pendenciasRes = await db.query("SELECT COUNT(*) FROM leads WHERE status IN ('Novo', 'Não respondeu', 'Em negociação')");
            additionalStats.pendencias = parseInt(pendenciasRes.rows[0].count);
        }
        res.json({
